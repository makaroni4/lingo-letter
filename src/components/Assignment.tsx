...
export default function Assignment() {
  ...
  const { incomingEmail, responseTopics, showExampleExamBadge } = useAppStore()

  return (
    <div className="mb-12">
      <h1 className="text-3xl font-bold mb-3 mt-8">{t("writing_test")}</h1>

      <div className="mb-8 text-lg">{t("you_have_received_an_email")}</div>
      ...
      <div className="mb-10">
        <ul className="list-disc list-inside">
          {responseTopics.map((topic) => {
            return (
              <li className="text-lg mb-4 last:mb-0 font-serif" key={topic}>
                {topic}
              </li>
            )
          })}
      ...
      <div className="text-lg">{t("writing_instructions")}</div>
    </div>
  )
}
